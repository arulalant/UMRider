...
def createTarBalls(path, oftype, today, utc, stephr=6):
    ...
    if oftype == 'analysis':    
        ...
        # # -m500 need to be include for pbzip2
        ...
        cmd = 'ssh ncmlogin3 "ssh %s rm -rf /home/incois/NCUM/osf/0.25/*%s*tar.gz"' % (nkn_server, y11Day)
        print cmd
        try:
            subprocess.call(cmd, shell=True)
        except Exception as e:
        ...
        cmd = "tar -c ./fcst*%s*.grb2 | %s  -v  -c -f -p32 > %s/fcst_glb_0.25_%s.tar.gz" % (today, pbzip2, '../TarFiles', today)
        print cmd
        subprocess.call(cmd, shell=True)        
