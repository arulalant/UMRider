...
def createTarBalls(path, oftype, today, utc, stephr=6):
    ...
    if oftype == 'analysis':    
        ...
        cmd = 'ssh ncmlogin3 "scp -p %s/ncum_anal_%s.tar.bz2  %s:/data/ftp/pub/outgoing/NCUM_INCOIS/OSF/0.5/"' % (tarpath, today, ftp_server)
        print cmd
        subprocess.call(cmd, shell=True)
        cmd = 'ssh ncmlogin3 "scp -p %s/ncum_anal_%s.tar.bz2  %s:NCUM/osf/0.5/"' % (tarpath, today, nkn_server)
        print cmd
        subprocess.call(cmd, shell=True)
        ...
        cmd = 'ssh ncmlogin3 "scp -p %s/ncum_fcst_%s.tar.bz2  %s:/data/ftp/pub/outgoing/NCUM_INCOIS/OSF/0.5/"' % (tarpath, today, ftp_server)
        print cmd
        subprocess.call(cmd, shell=True)
        cmd = 'ssh ncmlogin3 "scp -p %s/ncum_fcst_%s.tar.bz2  %s:NCUM/osf/0.5/"' % (tarpath, today, nkn_server)
        print cmd
        subprocess.call(cmd, shell=True)        
