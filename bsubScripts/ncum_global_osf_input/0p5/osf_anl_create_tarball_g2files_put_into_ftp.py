...
def createTarBalls(path, oftype, today, utc, stephr=6):
    ...
    if oftype == 'analysis':    
        ...
        cmd = "tar -c  %s | %s -v  -c -f -p32 -m500 > %s/ncum_anal_glb_0.5_%s.tar.bz2" % (anal_files, pbzip2, '../TarFiles', today)
        print cmd
        subprocess.call(cmd, shell=True)
        ...
        cmd = 'ssh ncmlogin3 "scp -p %s/ncum_anal_glb_0.5_%s.tar.bz2  %s:/data/ftp/pub/outgoing/NCUM_INCOIS/OSF/0.5/"' % (tarpath, today, ftp_server)
        print cmd
        subprocess.call(cmd, shell=True)
        cmd = 'ssh ncmlogin3 "scp -p %s/ncum_anal_glb_0.5_%s.tar.bz2  %s:NCUM/osf/0.5/"' % (tarpath, today, nkn_server)
        print cmd
        subprocess.call(cmd, shell=True)
