...
def createTarBalls(path, today, utc, stephr=3):    
    ...
    cmd = "tar -c ./fcst[1-24]*%s*.grb2 | %s  -v  -c -f -p32 > %s/fcst_day01_solar_ind_0.25_%s.tar.gz" % (today, pigz, '../TarFiles', today)
    print cmd
    subprocess.call(cmd, shell=True)
    
    # create forecast files tar file in parallel # -m500 need to be include for pbzip2
    cmd = "tar -c ./fcst[25-48]*%s*.grb2 | %s  -v  -c -f -p32 > %s/fcst_day02_solar_ind_0.25_%s.tar.gz" % (today, pigz, '../TarFiles', today)
    print cmd
    subprocess.call(cmd, shell=True)
    
    # create forecast files tar file in parallel # -m500 need to be include for pbzip2
    cmd = "tar -c ./fcst[49-72]*%s*.grb2 | %s  -v  -c -f -p32 > %s/fcst_day03_solar_ind_0.25_%s.tar.gz" % (today, pigz, '../TarFiles', today)
    print cmd
    subprocess.call(cmd, shell=True)
