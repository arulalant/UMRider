...
def putintoftp(today, outpath, oftype, utc):
    ...
    cmd = 'ssh ncmlogin3 "rsync -avrt %s %s:/data/ftp/pub/outgoing/NCUM_GLB/2.5/%s/"' % (gfiles, ftp_server, today)
    print cmd
    try:
        subprocess.call(cmd, shell=True)   
    except Exception as e:
    ...
    if oftype == 'analysis':
        ...
        cmd = 'ssh ncmlogin3 "ssh %s rm -rf /data/ftp/pub/outgoing/NCUM_GLB/2.5/%s"' % (ftp_server, y11Day)
        print cmd
        try:
            subprocess.call(cmd, shell=True)
        except Exception as e:
